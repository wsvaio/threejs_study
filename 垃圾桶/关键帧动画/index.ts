...
useAnimation(() => {
	...
	if (keys.has("w") || keys.has("a") || keys.has("d") || keys.has("s") || keys.has("1")) {
		setProp({
			obj: idleAction,
			key: "weight",
			val: 0,
	...
	const 对边 = camera.position.x - mesh.position.x;
	const 邻边 = camera.position.z - mesh.position.z;

	// mesh.rotation.y =  * runAction.weight;

	let r
    = 邻边 > 0 ? Math.PI + Math.atan(对边 / 邻边) : Math.atan(对边 / 邻边);

	// if (mesh.rotation.y - r > Math.PI ? -1 : 1)
	...
	if (keys.has("d"))
		r -= Math.PI / 2;
	if (keys.has("s"))
		r += Math.PI * (Math.random() > 0.5 ? 1 : -1);

	if (keys.has("w") || keys.has("a") || keys.has("d") || keys.has("s")) {
		setProp({
			obj: mesh.rotation,
			key: "y",
			val:
	...
	console.log((mesh.rotation.y / Math.PI) * 180);
	...
	controls.target.set(mesh.position.x, mesh.position.y + 1.5, mesh.position.z);
